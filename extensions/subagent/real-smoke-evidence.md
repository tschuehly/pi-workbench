...
A real Pi RPC lead loaded `extensions/subagent/index.ts`, used `openai-codex/gpt-5.4-mini` at low effort, and launched one `mechanics` Subagent with `background: true`. Runtime routing selected `anthropic/claude-haiku-4-5-20251001` at low effort for the child.
