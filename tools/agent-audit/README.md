...
The default Surface is a short informational explanation of Pi's context and model/tool loop, the independent Alignment and Checking guidance, and the difference between advertised skill metadata and a loaded skill body. Its Plan + tests example is explicitly illustrative, not a live selection. The complete saved-evidence explorer and JSON download remain under the collapsed **Explore saved instructions and evidence** disclosure. Cockpit reveal opens that disclosure before focusing an existing evidence Region or comment.

Inside the advanced explorer, positively recognized saved skill-catalog markup is omitted only from the readable instruction summary and remains in the exact diff. Unknown or unavailable request instructions/catalogs stay explicitly unknown or unavailable rather than becoming empty evidence. Baseline and target may be requests or preview sets. Preview controls derive from `dialDefinitions` and each preview's string-valued `dials` map; legacy `alignment`/`checking` fields remain supported. Values and combinations come only from saved previews. Each side also lists every saved combination directly, so sparse future dial sets remain reachable without silently changing another dial. A future dial needs data like `{ dialDefinitions:{tone:{label:"Tone",values:["plain","warm"]}}, previews:[{dials:{alignment:"Vibe",checking:"light",tone:"plain"}}] }`, not viewer code. Oversized instruction comparisons report measured sizes and defer to unchanged exact text rather than returning a partial diff.
