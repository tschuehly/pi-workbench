...
// Reused from the repository's Atelier kit and hardened for one sensitive audit export.
// Zero dependencies (node:http).
//
//   ROOT=.review/agent-audit/exports/<id> UI=$ROOT/index.html ACCESS_TOKEN_FILE=<sibling-token> node tools/agent-audit/server.mjs
// This audit copy requires one prepared export root and binds only to loopback.
//
...
if (!process.env.ROOT || !process.env.UI) throw new Error('ROOT and UI must name one prepared audit export');
const ROOT = fs.realpathSync(path.resolve(process.env.ROOT));
const PORT = Number(process.env.PORT || 4747);
const HOST = process.env.HOST || '127.0.0.1';
if (HOST !== '127.0.0.1' || !Number.isInteger(PORT) || PORT < 1 || PORT > 65535) throw new Error('agent-audit Atelier binds only to 127.0.0.1 on a valid port');
const UI = fs.realpathSync(path.resolve(process.env.UI));
if (UI !== ROOT && !UI.startsWith(ROOT + path.sep)) throw new Error('UI must be inside ROOT');
if (!process.env.ACCESS_TOKEN_FILE) throw new Error('ACCESS_TOKEN_FILE must name the private token beside the prepared export');
const TOKEN_PATH = path.resolve(process.env.ACCESS_TOKEN_FILE), tokenStat = fs.lstatSync(TOKEN_PATH);
if (tokenStat.isSymbolicLink() || !tokenStat.isFile() || (tokenStat.mode & 0o077)) throw new Error('access token must be an owner-only regular file');
const TOKEN_FILE = fs.realpathSync(TOKEN_PATH);
if (TOKEN_FILE === ROOT || TOKEN_FILE.startsWith(ROOT + path.sep)) throw new Error('access token must be outside the served export root');
const ACCESS_TOKEN = fs.readFileSync(TOKEN_FILE, 'utf8').trim();
if (!/^[A-Za-z0-9_-]{43}$/.test(ACCESS_TOKEN)) throw new Error('invalid access token');
const NAME = process.env.STORE || 'atelier';
if (!/^[a-zA-Z0-9_-]{1,80}$/.test(NAME)) throw new Error('invalid STORE name');
const DATA_DIR = path.join(ROOT, '.review');
const STORE_FILE = path.join(DATA_DIR, `${NAME}.json`);
await fsp.mkdir(DATA_DIR, { recursive: true, mode:0o700 });
await fsp.chmod(DATA_DIR, 0o700);
...
function persist(){
  ...
  saveTimer = setTimeout(async ()=>{
    ...
    try {
      ...
      await fsp.chmod(STORE_FILE, 0o600);
...
// ---- helpers ----
const SECURITY_HEADERS = { 'Content-Security-Policy':"default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self'; connect-src 'self'; img-src 'self' data:; object-src 'none'; base-uri 'none'; frame-ancestors 'none'", 'X-Content-Type-Options':'nosniff', 'Referrer-Policy':'no-referrer', 'Cross-Origin-Resource-Policy':'same-origin' };
const sendJSON = (res, code, obj) => { const b = JSON.stringify(obj); res.writeHead(code, { ...SECURITY_HEADERS, 'Content-Type':'application/json', 'Content-Length': Buffer.byteLength(b), 'Cache-Control':'no-store' }); res.end(b); };
const readBody = (req) => new Promise((resolve, reject) => { let d=''; req.on('data',c=>{ d+=c; if (d.length>1_000_000){ reject(new Error('request body too large')); req.destroy(); } }); req.on('end',()=>{ try{resolve(JSON.parse(d||'{}'));}catch{resolve({});} }); });
...
function authorized(req){
  ...
  const cookie = (req.headers.cookie || '').split(';').map(part => part.trim()).find(part => part.startsWith('agent_audit_access='))?.slice('agent_audit_access='.length);
  return tokenMatches(bearer) || tokenMatches(cookie);
}
...
function serveStatic(req, res, filePath, allowedRoot=ROOT){
  fs.realpath(filePath, (realError, realFile) => {
    if (realError || (realFile !== allowedRoot && !realFile.startsWith(allowedRoot + path.sep))){ res.writeHead(realError?404:403, SECURITY_HEADERS); res.end(realError?'not found':'forbidden'); return; }
    fs.stat(realFile, (err, st) => {
    if (err){ res.writeHead(404, SECURITY_HEADERS); res.end('not found'); return; }
    if (st.isDirectory()) realFile = path.join(realFile, 'index.html');
    fs.realpath(realFile, (nestedError, nestedFile) => {
      if (nestedError || (nestedFile !== allowedRoot && !nestedFile.startsWith(allowedRoot + path.sep))){ res.writeHead(nestedError?404:403, SECURITY_HEADERS); res.end(nestedError?'not found':'forbidden'); return; }
    fs.stat(nestedFile, (e2, st2) => {
      if (e2){ res.writeHead(404, SECURITY_HEADERS); res.end('not found'); return; }
      const type = MIME[path.extname(nestedFile).toLowerCase()] || 'application/octet-stream';
      const range = req.headers.range;
      if (range && (type.startsWith('video') || type.startsWith('audio'))){
        const m = /bytes=(\d*)-(\d*)/.exec(range) || [];
        const start = m[1] ? parseInt(m[1],10) : 0;
        const end = m[2] ? parseInt(m[2],10) : st2.size - 1;
        res.writeHead(206, { ...SECURITY_HEADERS, 'Content-Type':type, 'Content-Range':`bytes ${start}-${end}/${st2.size}`,
          'Accept-Ranges':'bytes', 'Content-Length': end-start+1, 'Cache-Control':'no-store' });
        fs.createReadStream(nestedFile, { start, end }).pipe(res);
      } else {
        res.writeHead(200, { ...SECURITY_HEADERS, 'Content-Type':type, 'Content-Length':st2.size, 'Cache-Control':'no-store' });
        fs.createReadStream(nestedFile).pipe(res);
      }
    });
    });
    });
...
async function handle(req, res){
  ...
  if (req.headers.host !== expectedHost) return sendJSON(res, 421, { ok:false, error:'unexpected Host' });
  if (req.method === 'POST' && req.headers.origin && req.headers.origin !== `http://${expectedHost}`)
    return sendJSON(res, 403, { ok:false, error:'cross-origin mutation rejected' });
  const url = new URL(req.url, `http://${expectedHost}`);
  const p = url.pathname;
  if ((req.method === 'GET' || req.method === 'HEAD') && p === '/' && tokenMatches(url.searchParams.get('token'))) {
    res.writeHead(303, { ...SECURITY_HEADERS, 'Location':'/', 'Set-Cookie':`agent_audit_access=${ACCESS_TOKEN}; HttpOnly; SameSite=Strict; Path=/`, 'Cache-Control':'no-store' });
    return res.end();
  }
  if (!authorized(req)) return sendJSON(res, 401, { ok:false, error:'local access token required' });
  ...
  if (p === '/atelier.mjs' || p === '/atelier.css') return serveStatic(req, res, path.join(HERE, p.slice(1)), HERE);
  const fp = resolveStatic(p);
  if (!fp){ res.writeHead(403, SECURITY_HEADERS); res.end('forbidden'); return; }
  serveStatic(req, res, fp);
}

server.listen(PORT, HOST, ()=>{
  console.log(`▶ Atelier  http://${HOST}:${PORT}/ (token required)`);
