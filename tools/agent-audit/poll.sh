...
if [ -z "${ACCESS_TOKEN_FILE:-}" ] || [ ! -f "$ACCESS_TOKEN_FILE" ]; then
  echo 'atelier poll: ACCESS_TOKEN_FILE must name the prepared export token' >&2; exit 2
fi
ACCESS_TOKEN="$(tr -d '\r\n' < "$ACCESS_TOKEN_FILE")"
case "$ACCESS_TOKEN" in (*[!A-Za-z0-9_-]*|'') echo 'atelier poll: invalid access token' >&2; exit 2;; esac
CURL_AUTH=(-H "Authorization: Bearer $ACCESS_TOKEN")
...
fetch_wake() {
  ...
  local resp
  resp="$(curl -s --max-time 30 "${CURL_AUTH[@]}" "${BASE_URL}/api/poll?cursor=${CURSOR}")"
