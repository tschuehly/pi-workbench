...
#           proposals/non-rejected states/clears) print NOTHING. Launched once at setup via
...
#
# Human-origin wake kinds (override with WAKE_KINDS env, csv): sent, flush, decision,
# explain-request, comment-rejected, and canonical command. Everything else is filtered out of
...
# The rest (reply, proposal, comment-state, ack, update-dismissed, clear) are echoes or human
...
WAKE_KINDS="${WAKE_KINDS:-sent,flush,decision,explain-request,comment-rejected,command}"
