...
// { name, threads:{[region]:[{id,text,tags,sev,queued,anchor?}]},   ← human drafts, client-owned
...
// Nothing is ever deleted. A Thread whose Region left the document stays in `threads`; the Surface
// shows it as archived.
...
const commentBody = (c) => { const { id, queued, ...rest } = c; return rest; };
...
async function handle(req, res){
  ...
  if (p === '/api/flush' && req.method === 'POST'){      // { region? } — dispatch every QUEUED comment
    const { region } = await readBody(req);
    const sentIds = [];
    for (const [key, arr] of Object.entries(store.threads)){
      if (region && key !== region) continue;
      for (const c of arr){
        if (c.queued && !store.sent[c.id]){
          store.sent[c.id] = new Date().toISOString();
          if (!store.commentState[c.id]) store.commentState[c.id] = { value:'open', ts:new Date().toISOString() };
          appendLog('sent', key, c.id, { comment: commentBody(c) });
          // A flush acknowledges only Regions where it actually dispatched human comments.
          if (store.changed[key]){ delete store.changed[key]; appendLog('ack', key, null, {}); }
          sentIds.push(c.id);
        }
      }
    }
    if (sentIds.length){ appendLog('flush', region||null, null, { ids:sentIds, count:sentIds.length }); persist(); }
    return sendJSON(res, 200, { ok:true, sentIds, cursor: store.seq });
  }
  ...
  if (p === '/api/propose' && req.method === 'POST'){    // { region, question, options[], threadId? }
    ...
    for (const pr of Object.values(store.proposals)){    // one active version per Region or Thread
      const sameThread = threadId && pr.threadId===threadId;
      const sameRegion = !threadId && !pr.threadId && pr.region===key;
      if ((sameThread||sameRegion) && pr.status==='open') pr.status='superseded';
    }
  ...
  if (p === '/api/clear' && req.method === 'POST'){      // { scope:'all'|'threads' }
    const { scope } = await readBody(req);
    const s = scope || 'all';
    if (!['all','threads'].includes(s)) return sendJSON(res, 400, { ok:false, error:'scope must be all|threads' });
    store.threads = {}; store.sent = {}; store.replies = {}; store.commentState = {};
    if (s === 'all'){ store.proposals = {}; store.updates = {}; store.changed = {}; }
    appendLog('clear', null, null, { scope:s }); persist();
    return sendJSON(res, 200, { ok:true, scope:s, cursor: store.seq });
  }
