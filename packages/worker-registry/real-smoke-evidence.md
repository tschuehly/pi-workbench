...
The machine-local registry directory and session identifiers are intentionally not recorded in the repository. Deterministic tests in `test/registry.test.js` and `packages/pi-execution-adapter/test/adapter.test.js` cover locking, dead-owner reclaim, inspection, continuation validation, and mismatch failure without real processes.
