...
signature_details="$(/usr/bin/codesign -dvv "${app_path}" 2>&1 || true)"
...
check_true "the staged app is signed before installation" /usr/bin/codesign --verify --strict "${app_path}"
check_true "the signature binds the bundle identifier" grep -Fq 'Identifier=works.pi.workbench' <<<"${signature_details}"
