...
For V1, the router resolves bindings for the interactive lead and bounded children by Cognitive Role without changing the attended posture. An attended Subagent or Worker dispatch may carry an exact owner-requested provider/model override as an exception; Workbench accepts that request from the caller but does not mechanically attest its owner provenance. Independent roles reject model overrides. Either dispatch may carry explicit Model Effort while the Cognitive Role still selects the model. The resolver validates model/effort availability, mapped quota, and any active routing overlay. Independent Subagents may exclude already-selected reviewer families; gateway authors require an exact model identity. The resolver selects a policy candidate outside the author and excluded families, and the adapter revalidates that evidence before launch.
