...
The governing AFK overlay supplies the owned Workbench worktree and roots. Standalone attended execution may create `exp/continuation-runtime-probes` only after the plan commit. Neither path uses `main` or another run's state. Put disposable probe implementations under a clearly experimental test directory, not `extensions/workstreams/`, so auto-discovery cannot mistake them for a supported interface.
