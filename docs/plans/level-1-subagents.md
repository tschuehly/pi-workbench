...
Static `model` fields are removed from bundled profiles. A model override is explicit per dispatch and never becomes profile, Subagent, or Worker identity.
...
12. a background launch returns a handle immediately; collect-by-ID on a running child or a finished Worker with an unsettled receipt returns a bounded snapshot without waiting or marking it collected; terminal outcomes coalesce into one bounded signal that triggers bulk collection, with status used only for diagnostics, without duplicate or post-collection notifications; and `status`, `list`, and `collect` reconcile the child within the session without ever letting it outlive the attended parent;
