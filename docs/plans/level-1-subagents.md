...
Detailed observations drive the rolling tool UI and, in interactive Pi, one line of at most 100 characters on the shared **Active** surface above the editor for every active Subagent or Worker dispatch. The normalized current action comes first, followed by compact identity, Cognitive Role, and model tags; the objective appears only before real activity arrives. A terminal background execution becomes one bounded **Done** summary until the next interactive input, and Worker success appears only after its receipt settles. These presentation updates do not enter parent Model Context. The parent receives only terminal status, final text, child profile, Cognitive Role, resolved provider/model/effort, effective quota admission and telemetry status, and Pi session identifier. V1 does not expose arbitrary JSON Schema or automatic correction turns. Usage is observed, but custom token and cost enforcement remains deferred.
