...
worker_dispatch { workerId: string; task: string;
                  cognitiveRole: CognitiveRole; modelOverride?: string; effort?: ModelEffort;
...
the model and Model Effort. As with a Subagent, an optional owner-requested `modelOverride` selects
one available `<provider>/<model>` for that dispatch; an optional `effort` independently overrides
the role effort while the role still selects the model. Routing fails closed on invalid syntax,
missing quota-provider mappings, unavailable models or effort, independent-role model overrides,
exhausted fresh quota, or a model outside an active routing overlay. Continuity of context never
pins a model: the adapter sets and verifies the resolved binding on every resumed dispatch, so one
worker may use different models across dispatches.
...
6. every dispatch uses a fresh verified binding, including a changed model or effort on a resumed session; an explicit model override follows mapped selected-provider quota telemetry, explicit effort is validated against the selected model, active overlays remain enforced, and the resolved binding appears in launch and terminal evidence;
