...
79. **Operating Levels are concepts, not an adoption roadmap.** Level 1 is attended human–Pi pairing; Level 2 adds bounded delegation; Level 3 adds an approved contract and bounded independent review; Level 4 adds deterministic managed execution. Higher numbers are different postures, not quality rankings.
80. **V1 implements Level 1 only.** One human pairs with one interactive Pi under continuous Human Attention. No advisor, Subagent, delegated semantic work, background model turn, or unattended execution is launched.
81. **The V1 model binding resolves one interactive Pi.** Model and Model Effort may vary without changing the Level 1 posture. Bindings for delegated roles are outside V1.
82. **V1 has no Level selector.** PI WEB presents the Level 1 attended boundary where sessions start. Any later level requires a separate decision, implementation plan, and truthful presentation of its authority and recovery guarantees.
