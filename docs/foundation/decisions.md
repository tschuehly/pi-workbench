...
24. **V1 is a PI WEB Workstream vertical slice.** PI WEB consumes the Workstream protocol and supports attended human–Pi pairing without owning Workstream state.
...
82. **The current V1 has no Working Mode control.** *(Revised 2026-08-27.)* Sessions currently start in the attended posture without a mechanical alignment gate or persistent mode display. The owner-settled intended control is specified in [Working Mode](working-mode.md); implementing it requires the separate [Working Mode implementation plan](../plans/working-mode.md) and truthful presentation of what is and is not enforced.
...
69. **Working Mode has independent Alignment and Checking axes.** *(Owner-settled on 2026-08-27; replaces the 2026-08-26 single-axis Operating Level design after an attended-session failure showed that strong Checking does not prevent wrong-direction mutation.)* `Alignment: Vibe | Plan | Spec` controls the minimum Shared Understanding required before mutation: Vibe permits one owner-inspectable slice, Plan requires an accepted concise outcome, approach, boundaries, and evidence statement, and Spec requires accepted behavior, constraints, and acceptance evidence while implementation strategy remains adaptive. `Checking: light | tests | adversarial` independently controls verification. Every task begins read-only in Discovering with both values unselected; Pi recommends and the owner chooses before mutation. The current configuration is always visible, survives session resume, and changes prospectively. Grilling may produce Plan or Spec, and every human-facing mode interaction applies `write-for-humans`. Working Mode configures behavior, never permission. Further axes are added one at a time from observed need. Alignment selection, gating, and persistent display are not implemented; Decision 82 records current behavior and the [Working Mode plan](../plans/working-mode.md) defines the proposed sequence. See [Working Mode](working-mode.md).
...
33. **The core V1 interface is the Workstream Store.** Its six operations, deterministic projection, typed PI WEB transport, attended session association, checkpoint confirmation, and restart behavior define the initial implementation. Managed lifecycle and execution interfaces remain unbuilt concepts.
