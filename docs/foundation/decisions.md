...
92. **The first Workbench client root is a minimal Chat composition.** *(Owner-revised on 2026-08-28.)* The existing AppKit wrapper hosts one selected Chat per window. The web client may reuse PI WEB controllers and leaf surfaces directly; shell profiles, plugin-owned navigation, and the registry-driven Pi menu are legacy evidence rather than the target composition.
95. **The unified-shell candidate is stopped and archived.** *(Owner-revised on 2026-08-28.)* Its controlled-session harness, runtime isolation, complete session identity, reconnect checks, and typed Workstream mechanics remain reusable evidence. The separately routed Workstream Store lock-recovery experiment retains approval because durable-worker locking still depends on its finding. Shell fidelity, unified navigation, continuation UI, message-tree, child-card, installation, and release phases create no current scope; each may return only from observed friction in a delivered client slice.
