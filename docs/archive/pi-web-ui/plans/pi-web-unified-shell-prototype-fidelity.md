...
## Execution record and attended continuation

The owner approved every phase and the six Phase 6 Material Question outcomes on 2026-08-08; Decisions 94 and 95 record that approval. Exact amendments may be implemented when they stay within those outcomes; new semantic choices require Human Attention.

The first execution attempt ran unattended and died mid-Phase 3: its isolated worktrees were lost with the uncommitted Phase 3 candidate, and only the committed Phase 1 and Phase 2 candidates survived. Those candidates were adopted attended onto the normal integration branches and re-verified:

- Phase 1 (controlled fixture and acceptance harness): PI WEB commit `05dc04a`, Workbench commit `bc792c9`.
- Phase 2 (transactional shell profiles and the Workbench profile declaration): PI WEB commit `8644d99`, Workbench commit `73a59da`.
- Attended re-verification on 2026-08-08: PI WEB `npm run verify` (2,596 passed, 3 skipped, 290 files) and `npm run build` passed; Workbench integration `check` passed (88 tests); a fresh controlled browser acceptance run passed 9 of 10 checks with truthful `partial` status — `CURRENT_COMPOSITION_MISMATCH` remains the expected Phase 3 red assertion, and `LIVE_ASK_UNREACHABLE`/`CONTEXT_USAGE_UNREACHABLE` remain documented later-phase fixture blockers.

Execution continues attended from Phase 3. The lost attempt's Phase 3 scope, independent-review findings (one blocker, four highs), and repair order are preserved in the [archived continuation handoff](../../../research/reports/pi-web-unified-shell-afk-phase3-handoff-2026-08-08.md); treat them as the starting defect list, never as implemented behavior. All remaining work uses isolated experiment worktrees and temporary runtime/install roots. Publication, credentials, normal PI WEB/Pi/Workbench data, the installed live app, and live services stay outside experiment authority; **evidenced inapplicable to isolated candidate** applies only to physical-device, publication, live-install replacement, credential, or owner-comparison actions outside that authority, and each becomes a named Human Attention item — technical or prototype-interaction gaps cannot use that disposition. Phase 4 labels proposed screenshot baselines experimental and defers the owner's visual judgment to the single final Acceptance item. Phase 9 runs installer, status/doctor, update, disable, rollback, uninstall, built-app launch, and sessiond-continuity drills only against a copied app and fake `HOME`/LaunchAgents/config/data/session roots owned by the experiment; it never invokes `launchctl`/`systemctl`, registers a service, restarts the normal daemon, opens the installed app, or writes `~/Library`. Phase 10's installed-app exit means the isolated copied candidate. No document changes to `released` before owner Acceptance.
...
[`customization-plan.md`](../integration/customization-plan.md), rather than exposing arbitrary
