...
75. **The graphical client is attention-first by default.** Its entry surface answers what needs the owner's judgment now, separates required action from work progressing independently, and makes completed outcomes and re-entry state visible. Each focused Attention Item leads with the required judgment, materiality, recommendation, consequences, deferral behavior, and typed actions before progressively disclosing evidence. This cognitive-accessibility posture applies to every user rather than requiring a separate mode.
76. **Graphical-shell extensibility is upstream-first.** When PI WEB lacks a UI capability needed by the Workbench, first prove the need in a bounded adapter and contribute a generic, stable extension point or API upstream. PI WEB owns reusable shell capabilities; the Workbench adapter owns Run, attention, evidence, authority, and Review Surface semantics. Do not expose private component internals or make every shell component replaceable. Maintain a bounded fork only when required shell behavior cannot be expressed through accepted stable extension points and the shared selection fixture demonstrates that the maintenance cost is justified.
77. **Pi Workbench is Pilot Zero for staged self-hosting.** Each implemented vertical slice must make the next Pi Workbench slice more self-hosted. Adoption advances from shadow projection, through controller-assisted operation and bounded scout dispatch, to isolated Ship execution and primary-workflow use only as the corresponding authority, evidence, validation, recovery, and durability mechanisms become trustworthy. PhotoQuest remains the first external delivery pilot. Self-hosting supplies product evidence but never replaces deterministic validation, independent review, Git history, or the terminal recovery path.
